...
import { DynamoDB as DynamoDBActions, Logs as LogsActions, IoT as IoTActions, Greengrass as GreengrassActions } from 'cdk-iam-actions/lib/actions'
...
export class DeviceServices extends Construct {
	...
	constructor (scope: Construct, id: string, props: DeviceServicesProps) {
				...
				devicesServiceIAMPolicy: new PolicyDocument({
					statements: [
						new PolicyStatement({
							...
							resources: [
								Stack.of(this).formatArn({ service: 'dynamodb', resource: 'table', resourceName: settingTable.tableName }),
								Stack.of(this).formatArn({ service: 'dynamodb', resource: 'table', resourceName: deviceTable.tableName }),
								Stack.of(this).formatArn({ service: 'dynamodb', resource: 'table', resourceName: deviceTypeTable.tableName }),
							],
						}),
						new PolicyStatement({
							effect: Effect.ALLOW,
		...
		const lambdaFunction = new LambdaFunction(scope, 'LambdaFunction', {
			...
			code: NpmCode.fromNpmPackageDir(this, path.join(SOURCE_DIR, '/services/devices')),
		})
