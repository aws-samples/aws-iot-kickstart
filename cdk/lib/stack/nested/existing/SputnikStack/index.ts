...
import { uniqueIdHash, namespaced, namespacedBucket } from '../../../../utils/cdk-identity-utils'
...
import { DeviceStack } from '../../../root/DeviceStack'
...
import { DefaultsStack } from './DefaultsStack'
...
export class SputnikStack extends NestedStack {
	...
	readonly defaultsStack: DefaultsStack
	...
	constructor (scope: Construct, id: string, props: SputnikStackProps) {
		 ...
		 * ROLES & POOLICIES
		***************************************************************************/
		this.greengrassServiceIAMRole = new iam.Role(this, 'greengrassServiceIAMRole', {
			assumedBy: new iam.ServicePrincipal('greengrass.amazonaws.com'),
			managedPolicies: [iam.ManagedPolicy.fromAwsManagedPolicyName('service-role/AWSGreengrassResourceAccessRolePolicy')],
		})

		this.greengrassGroupsIAMRole = new iam.Role(this, 'greengrassGroupsIAMRole', {
			roleName: namespaced(this, `${id}-GreengrassGroupsIAMRole`),
			assumedBy: new iam.ServicePrincipal('greengrass.amazonaws.com'),
		})

		this.IoTPolicyForGreengrassCores = new iot.CfnPolicy(this, 'IoTPolicyForGreengrassCores', {
			policyDocument: {
				'Version': '2012-10-17',
				'Statement': [
					{
		 ...
		 * RESOURCES
		***************************************************************************/
		const defaultsBucketName = namespacedBucket(this, 'defaults')

		this.lambdaHelpersStack = new IncludeStack(this, 'CFStackForLambdaHelpers', {
			templateFile: getTemplateFile('lambda-helpers'),
			parameters: {
				customResourceS3Helper: new NpmCode(this, 'customResourceS3Helper', {
					path: path.join(RESOURCES_DIR, 'cf-helper-s3'),
				}),
				utilsCustomResourceLambdaFunction: new NpmCode(this, 'utilsCustomResourceLambdaFunction', {
					path: path.join(RESOURCES_DIR, 'utils'),
				}),
				'defaultsBucketArn': `arn:aws:s3:::${defaultsBucketName}`,
				...
				'deviceTypesTable': persistent.deviceManagementStack.deviceTypeTable.tableName,
				'deviceBlueprintsTable': persistent.deviceManagementStack.deviceBlueprintTable.tableName,
				'systemsTable': persistent.deviceManagementStack.systemTable.tableName,
				'systemBlueprintsTable': persistent.deviceManagementStack.systemBlueprintTable.tableName,
		...
		this.lambdaServiceStack = new IncludeStack(this, 'CFStackForLambdaServices', {
			templateFile: getTemplateFile('lambda-services'),
			parameters: {
				tenantRoleArn: cognitoStack.tenantRole.roleArn,
				deploymentsServiceLambdaFunction: new NpmCode(this, 'deploymentsServiceLambdaFunction', {
					path: path.join(SERVICES_DIR, 'deployments'),
				}),
				settingsServiceLambdaFunction: new NpmCode(this, 'settingsServiceLambdaFunction', {
					path: path.join(SERVICES_DIR, 'settings'),
				}),
				systemsServiceLambdaFunction: new NpmCode(this, 'systemsServiceLambdaFunction', {
					path: path.join(SERVICES_DIR, 'systems'),
				}),
				justInTimeOnBoardingServiceLambdaFunction: new NpmCode(this, 'justInTimeOnBoardingServiceLambdaFunction', {
					path: path.join(SERVICES_DIR, 'just-in-time-on-boarding'),
				}),
				...
				'deviceTypesTable': persistent.deviceManagementStack.deviceTypeTable.tableName,
				'deviceBlueprintsTable': persistent.deviceManagementStack.deviceBlueprintTable.tableName,
				'deploymentsTable': persistent.deviceManagementStack.deploymentTable.tableName,
				'systemsTable': persistent.deviceManagementStack.systemTable.tableName,
				'systemBlueprintsTable': persistent.deviceManagementStack.systemBlueprintTable.tableName,
		...
		this.appSyncStack = new IncludeStack(this, 'CFStackForAppSync', {
			...
			parameters: {
				...
				'settingsTable': persistent.deviceManagementStack.settingTable.tableName,
				'dataStoreTable': persistent.deviceManagementStack.dataStoreTable.tableName,
				'deviceTypesTable': persistent.deviceManagementStack.deviceTypeTable.tableName,
				'deviceBlueprintsTable': persistent.deviceManagementStack.deviceBlueprintTable.tableName,
				'deploymentsTable': persistent.deviceManagementStack.deploymentTable.tableName,
				'systemsTable': persistent.deviceManagementStack.systemTable.tableName,
		 ...
		 * CUSTOM RESOURCES
		***************************************************************************/
		this.greengrassAssociateServiceRoleToAccount = new CustomResource(this, 'greengrassAssociateServiceRoleToAccount', {
			resourceType: 'Custom::Lambda',
			serviceToken: utilsFunction.functionArn,
			properties: {
		...
		this.websiteConfig = new CustomResource(this, 'websiteConfig', {
			...
			properties: {
				...
				file: {
					...
					'IOT_COGNITO_POLICY': persistent.cognitoStack.websiteCognitoIoTPolicy.policyName,
		...
		***************************************************************************/
		if (loadDefaults) {
			this.defaultsStack = new DefaultsStack(this, 'DefaultsStack', {
				bucketName: defaultsBucketName,
				greengrassGroupsRole: this.greengrassServiceIAMRole,
				persistent,
				utilsFunction,
			})
		}
